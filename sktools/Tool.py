...
choice = input(Fore.MAGENTA + f"[binner] Please tell option to start >>>  ")
...
def agasthya():
   ...
   if __name__ == '__main__':
      ...
      while(True):
         try:
            thrd = int(input(Fore.GREEN +"[binner] THREADS : "+Fore.RESET))
            break
         except:
            pass
      while(True):
         try:
            inpFile = input(Fore.GREEN +"[binner] URLS PATH: "+Fore.RESET)
...
if choice == "9":
   ...
   def reverse(cidr):
      ...
      try :
            ...
            for xc in urxc:
               ...
               if ">>>>>>>>>>>>>>>>>>urx" in xc:
                  urls = urls+urxc[nm]+"\n"
            with open(os.path.join('', 'reversedbybinner.txt'), 'a') as output:
               piro =(f'{urls.replace("<td>","").replace ("</td>","")}')
               output.write(f'{piro}')
            print(f"[binner] REVERSED : {cidr}", end='\n')
   ...
   with open('ips.txt', 'w') as f:
      f.write(IPS)
   print("[REVERSD] Urls Stored in reversedbybinner.txt ")
...
def d2i():
    ...
    lists = input( yellow + '\n [binner] Domains List Path >>>'.format(o, g, o, g))
    thread = input(green + '\n [binner] Thread >>> '.format(o, g, o, r))
    print('')
    try:
